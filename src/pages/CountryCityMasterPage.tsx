...
import {
  ...
  updateCity,
  updateCountry,
} from "../services/countryCityService";
...
export const CountryCityMasterPage = () => {
  ...
  const loadDataFromAPI = async () => {
    try {
      ...
      const mapped = apiData
        .filter((c: any) => c.status !== "inactive") // Hide inactive
        .map((c: any) => ({
          ...
          status: c.status, // Keep actual backend status
  ...
  const handleAddCity = async () => {
    ...
    const payload = {
      cityName,
      cityCode: cityFormData.cityCode,
      postalCode: cityFormData.cityCode,
  ...
  const confirmDeleteCountry = async () => {
    ...
    try {
      const res = await inactivateCountry(Number(countryToDelete));

      if (res.message?.toLowerCase().includes("inactive")) {
        toast.error(res.message);
      } else {
        toast.success("Country marked as inactive");
      }

      // Refresh the data so inactive countries disappear
      await loadDataFromAPI();
      await loadCountriesFromAPI();
    } catch (err) {
      console.error("Inactivate failed:", err);
      toast.error("Failed to inactivate country");
    }
  ...
  const handleUpdateCityStatus = (
    countryId: string,
    cityId: string,
    newStatus: "active" | "inactive" | "deprecated"
  ) => {
  ...
  const handleUpdateCountry = async () => {
    ...
    try {
      const payload = {
        ...
        countryName:
          editCountryFormData.displayName || editCountryFormData.name,
  ...
  const handleUpdateCity = async () => {
    ...
    const payload = {
      ...
      cityCode: editCityFormData.code,
      postalCode: editCityFormData.code,
      countryId: Number(selectedCountry.id),
      createdBy: editingCity.createdBy ?? 1,
      updatedBy: 1,
    };
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      <Card className="bg-white border-[#5B9BD5]/30 hidden lg:block rounded-2xl">
        <CardHeader className="border-b bg-gradient-to-r from-sky-50 to-blue-50 pb-6">
          ...
          <CardDescription>
            ...
            {filteredCountries.length !== 1 ? "ies" : "y"} found
          </CardDescription>
        </CardHeader>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-gradient-to-r from-sky-50 to-blue-50">
                  <TableHead style={{ color: "#5B9BD5" }} className="px-6 py-4">
                    Country
                  </TableHead>
                  <TableHead style={{ color: "#5B9BD5" }} className="px-6 py-4">
                    Code
                  ...
                  filteredCountries.map((country) => (
                    ...
                    >
                      ...
                      <TableCell className="px-6 py-6">
                        <Badge
                          variant="outline"
                          className="border-[#5B9BD5]/30 text-[#5B9BD5] font-medium"
                        >
                          {country.code}
                        </Badge>
                      </TableCell>
                            ...
                            country.cities.map((city) => (
                              ...
                              >
                                <div className="flex items-center gap-4 flex-1 min-w-0">
                                  ...
                                  <Badge
                                    variant="outline"
                                    className="text-xs border-[#5B9BD5]/30 text-[#5B9BD5] font-medium"
                                  >
                                    {city.code}
                                  </Badge>
                                </div>
                  ...
                  <div className="flex items-center gap-3 flex-1">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <Badge
                          variant="outline"
                          className="border-[#5B9BD5]/30 text-[#5B9BD5] text-xs"
                        >
                          {country.code}
                        </Badge>
                        {getStatusBadge(country.status)}
              ...
              <CardContent className="pt-3 space-y-2">
                ...
                {country.cities.length === 0 ? (
                  ...
                  country.cities.map((city) => (
                    <div
                      key={city.id}
                      className="p-3 bg-white rounded-lg border border-sky-200"
                    >
      ...
      <Dialog
        ...
        onOpenChange={setIsAddCountryDialogOpen}
      >
        <DialogContent className="sm:max-w-[600px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Select a country from the dropdown and optionally provide a custom
              display name
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <p className="text-xs text-gray-500">
                Select from the complete list of world countries
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setCountryFormData({
                    ...countryFormData,
                    manualCountryName: e.target.value,
                  })
                }
              />
              <p className="text-xs text-gray-500">
                Override the default country name with a custom label (e.g.,
                "KSA" instead of "Saudi Arabia")
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value: any) =>
                  setCountryFormData({ ...countryFormData, status: value })
                }
              >
                  ...
                  <div className="text-xs text-blue-800">
                    ...
                    {countryFormData.manualCountryName ||
                      WORLD_COUNTRIES.find(
                        (c) => c.code === countryFormData.selectedCountry
                      )?.name}
                    "
                    {countryFormData.manualCountryName && (
                      <span>
      ...
      <Dialog
        ...
        onOpenChange={setIsEditCountryDialogOpen}
      >
        <DialogContent className="sm:max-w-[600px]">
          <DialogHeader>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-gray-500">
                Country name cannot be changed
              </p>
            </div>
              ...
              <Card className="p-3 bg-blue-50/50 border-blue-200">
                <div className="flex items-start gap-2">
                  ...
                  <div className="text-xs text-blue-800">
                    ...
                    "{editCountryFormData.displayName}" (Original:{" "}
                    {editCountryFormData.name})
                  </div>
                </div>
              </Card>
            )}
        ...
        <DialogContent className="sm:max-w-[600px]">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-gray-500">
                Override the default city name with a custom label for
                localization
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-gray-500">
                Internal reference code (2-5 characters)
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value: any) =>
                  setEditCityFormData({ ...editCityFormData, status: value })
                }
              >
              ...
              <Card className="p-3 bg-blue-50/50 border-blue-200">
                <div className="flex items-start gap-2">
                  ...
                  <div className="text-xs text-blue-800">
                    ...
                    {editCityFormData.displayName}" (Original:{" "}
      ...
      <Dialog
        ...
        onOpenChange={setIsAddCityDialogOpen}
        modal={true}
      >
        <DialogContent className="sm:max-w-[600px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Select a city from the dropdown or enter a custom city name if not
              available
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <Label htmlFor="citySelect">
                ...
                <span className="text-gray-500">
                  (Optional if manual name provided)
                </span>
              </Label>
              <Select
            ...
            <div className="space-y-2">
              <Label htmlFor="manualCityName">
                Manual City Name{" "}
                <span className="text-gray-500">
                  (Required if city not in dropdown)
                </span>
              </Label>
              <Input
                id="manualCityName"
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value: any) =>
                  setCityFormData({ ...cityFormData, status: value })
                }
              >
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={cityFormData.cityCode.trim().length < 2}
              style={{ background: "#5B9BD5" }}
              className="text-white"
            >
              Add City
      ...
      >
        ...
        <DialogContent className="sm:max-w-[400px] z-50">
          <DialogHeader>
            <DialogTitle className="text-red-600">Confirm Delete</DialogTitle>
            <DialogDescription>
              Are you sure you want to delete this country? All its cities will
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setIsDeleteCountryOpen(false)}
            >
              No
            </Button>
